...
async function generatePDF(options) {
  let {
    ...
    checkboxIndexes = [1, 2],
    outputDir = path.resolve('./download'),
    headless = false
  } = options;
  ...
  // 确保输出目录存在并格式化路径
  if (outputDir) {
    // 使用 path.resolve 确保绝对路径，避免相对路径问题
    let normalizedOutputDir = path.resolve(outputDir);
    ...
    if (!fs.existsSync(normalizedOutputDir)) {
      try {
        ...
        logger.info('创建输出目录成功', { 
          ...
          absolutePath: path.resolve(normalizedOutputDir)
        });
      } catch (mkdirError) {
        ...
        throw new Error(`无法创建输出目录: ${normalizedOutputDir}。错误: ${mkdirError.message}`);
      }
    } else {
      logger.info('输出目录已存在', { 
        outputDir: normalizedOutputDir,
  ...
  try {
    ...
    try {
      ...
      if (textContent) {
        ...
        if (parts.length >= 4) {
          ...
          // 如果没有-或分割后项数不足4项，则fileName为textContent
          fileName = textContent;
          logger.success('文件名使用原始文本内容', { fileName: textContent });
        }
      } else {
    ...
    try {
      ...
      try {
        ...
        logger.info('第一个PDF文件写入成功', { 
          filePath: filePath1, 
          fileSize: fileSize,
          platform: process.platform,
          absolutePath: path.resolve(filePath1)
        });
      } catch (writeError) {
        logger.error('第一个PDF文件写入失败', { 
          filePath: filePath1,
          error: writeError.message,
          platform: process.platform,
          absolutePath: path.resolve(filePath1)
        });
        throw new Error(`无法写入文件: ${filePath1}。错误: ${writeError.message}`);
      }
    } catch (error) {
    ...
    try {
      ...
      if (checkboxes.length >= Math.max(...checkboxIndexes) + 1) {
        ...
        try {
          ...
          logger.info('第二个PDF文件写入成功', { 
            filePath: filePath2, 
            fileSize: fileSize2,
            platform: process.platform,
            absolutePath: path.resolve(filePath2)
          });
        } catch (writeError) {
          logger.error('第二个PDF文件写入失败', { 
            filePath: filePath2,
            error: writeError.message,
            platform: process.platform,
            absolutePath: path.resolve(filePath2)
          });
          throw new Error(`无法写入文件: ${filePath2}。错误: ${writeError.message}`);
        }
      } else {
